  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (notification) {
        ...
        await acceptNotification(db, cfg, body.encResp, api);
      ...
      let q = findQuote(db, cfg, body.quoteToken);
      if (req.url === '/session') return send(200, beginCheckout(db, cfg, q));
      if (req.url === '/status') q = await refreshQuote(db, cfg, q, api);
